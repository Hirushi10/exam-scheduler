...
def load_all_schedules():
    conn = get_db_connection()
    df = pd.read_sql("SELECT id, date AS Date, time AS Time, subject AS Subject, hall AS Hall, supervisor AS Supervisor, invigilators AS Invigilators, semester_key, student_count FROM exam_assignments", conn)
    conn.close()
    return df
...
def generate_word_report(df, title_text):
    ...
    # Drop internal columns for clean report output compilation
...
# Get total unique list of names from Excel
all_staff_names = staff_df['Name'].dropna().unique().tolist()

# Parse Hall Capacity configuration maps from Column B
hall_capacities = {}
...
with col_control:
    ...
    # Dynamic Expected Student Count Field Input Descriptor
    default_students = 0
    ...
    # Supervisor configuration dropdown
    ...
    # Form Action Buttons execution queries
    if st.session_state.edit_mode:
...
# COLUMN 2: VISUAL HALL BOXES / CARDS WITH CAPACITY COUNTERS (Middle)
with col_display:
    ...
    for idx, hall_name in enumerate(all_halls):
        ...
        max_cap = hall_capacities.get(hall_name, "N/A")
        ...
        with target_col:
            ...
            if not hall_record.empty:
                ...
                # Overcrowding safety metrics logic validation
                ...
                st.markdown(f"""
                    <div class="hall-box" style="border-left: {box_border}; background-color: {box_bg};">
                        ...
                        <div class="hall-title" style="color: {title_color};">{title_prefix} {hall_name}</div>
...
if not schedule_df.empty:
    ...
    for index, row in schedule_df.iterrows():
        with st.container():
            ...
            with m_col4 if 'm_col4' in locals() else r_col4:
